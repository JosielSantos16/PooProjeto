...
public class Main extends javax.swing.JFrame {
    ...
    try {
        if (servico.checkDuplicateUser(usuario.getNome())) {
            ...
            mostrarMensagem(Mensagem.MessageType.SUCCESS, "Usuário cadastrado com sucesso");
        }
    ...
    System.out.println("Tentativa de login com CPF: " + dados.getCpf()); // Adicione esta linha para verificar o CPF
    try {
        Funcionario usuario = servico.login(dados);
        if (usuario != null) {
            System.out.println("Login bem-sucedido. Nome do usuário: " + usuario.getNome());
            this.dispose();
            MainSystem.main(usuario);
        } else {
            System.out.println("Falha no login. CPF e senha incorretos.");
